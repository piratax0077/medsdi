...
const BasicButton = () => {
    ...
    const outlineButtons = buttonVariants.map((variant, idx) => (
        <OverlayTrigger key={idx} overlay={<Tooltip id="tooltip2">{'outline-' + variant}</Tooltip>}>
    ...
    const glowButtons = buttonVariants.map((variant, idx) => (
        <OverlayTrigger key={idx} overlay={<Tooltip id="tooltip3">{'btn-glow-' + variant}</Tooltip>}>
    ...
    const basicDropdownButton = buttonOptions.map((button) => {
        ...
        return (
            <DropdownButton title={title} variant={button.variant} id={`dropdown-variants-${button.variant}`} key={button.variant}>

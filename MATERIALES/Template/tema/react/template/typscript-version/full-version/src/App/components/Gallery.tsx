...
const Gallery = (props: GalleryProps) => {
    ...
    const openLightbox = (event: React.MouseEvent<HTMLElement>, index: number) => {
    ...
    const handleClickItem = () => {
        ...
        if (data.currentItem === items!.length - 1) return;
        gotoNext();
    };
    const renderGallery = () => {
        if (data.itemType === 'images') {
            ...
            if (props.singleItem) {
                gallery = images
                    ...
                    .map((obj, i) => {
                        return (
                            <a href={obj.src} className={css(classes.thumbnail)} onClick={(e) => openLightbox(e, i)} key={i}>
            ...
            if (!texts) return;
            if (props.singleItem) {
                gallery = texts.map((text: string, i: number) => {
                    return (
            ...
            if (props.singleItem) {
                gallery = videos.map((videoId, i) => {
                    ...
                    return (
                        <a href={DEMO.BLANK_LINK} className={css(classes.video_thumbnail)} key={i} onClick={(e) => openLightbox(e, i)}>
                            <div className="img-thumbnail">
    ...
    return (
        <div className="section">
            ...
            <Lightbox
                ...
                items={{ type: data.itemType, items: data.itemType === 'videos' ? props.videos : props.images }}
                isOpen={data.lightboxIsOpen}

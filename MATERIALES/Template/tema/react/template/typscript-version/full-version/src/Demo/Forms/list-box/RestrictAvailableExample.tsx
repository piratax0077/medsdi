...
const RestrictAvailableExample = () => {
    ...
    const onPlanetChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const planet = event.currentTarget.value;
        setPlanet(planet);
    };
    const renderPlanets = () => {
        return Object.keys(planets).map((planet_) => (
            <Form.Group key={planet_} className="d-inline">

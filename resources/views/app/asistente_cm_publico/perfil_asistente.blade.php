											...
											<div class="profile-dp">
												<div class="position-relative d-inline-block">
													<img class="img-radius img-fluid wid-100" id="profile-image"
                                                        src="{{ $asistente->foto_perfil ? asset('storage/' . $asistente->foto_perfil) : asset('images/iconos/usuario_asistente.svg') }}"
                                                        alt="User image"></div>
												<div class="overlay">
...
@section('page-script')
    ...
    <script>
        ...
        function subir_foto_perfil(file) {
            ...
            $.ajax({
                url: '{{ route("asistente.actualizar.foto") }}', // Necesitas crear esta ruta
                method: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                error: function(xhr, status, error) {
                    ...
                    document.getElementById('profile-image').src = '{{ $asistente->foto_perfil ? asset("storage/" . $asistente->foto_perfil) : asset("images/iconos/usuario_asistente.svg") }}';
                }
            });
        }
        ...
        function eliminar_foto_perfil() {
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: '{{ route("asistente.eliminar.foto") }}', // Necesitas crear esta ruta
                        method: 'POST',
                        data: {
                            '_token': '{{ csrf_token() }}'
                        },
                        success: function(response) {
                            if (response.success) {
                                ...
                                document.getElementById('profile-image').src = '{{ asset("images/iconos/usuario_asistente.svg") }}';
                            } else {
                                swal({
                                    title: "Error",

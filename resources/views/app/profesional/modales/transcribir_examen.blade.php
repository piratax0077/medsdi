    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success align-middle" onclick="registrat_transcripcion()"; data-dismiss="modal">Guardar - Confirmar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function abrir_transcripcion_examen(id_hora_medica)
    {
        ...
        if(id_hora_medica!=='')
        {
            ...
            let url = '{{ route('profesional.cargar.examen.transcripcion') }}';
            $.ajax({
    ...
    function registrat_transcripcion()
    {
        ...
        let url = "{{ route('profesional.registro.examen.transcripcion') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: data,
            ...
            Dropzone.options.misImagenes = {
                init:function()
                {
                    ...
                    if(lista_imagenes['rfl'].length != 0)
                    {
                        ...
                        {
                            let mockFile = { name: value[1], size: 1000, accepted: true, status: 'success', ext: value[3] };
                ...
                init:function()
                {
                    ...
                    if(lista_imagenes['eda'].length != 0)
                    {
                        $.each(lista_imagenes['eda'], function (index, value)
                        {
                            ...
                            myDropzone_eda.files.push(mockFile);
                        });
                    }
                },
                url: "{{ route('profesional.imagen.carga') }}",
                ...
                init:function()
                {
                    ...
                    if(lista_imagenes['edb'].length != 0)
                    {
                        $.each(lista_imagenes['edb'], function (index, value)
                        {
                            ...
                            myDropzone_edb.files.push(mockFile);
                        });
                    }
                },
                url: "{{ route('profesional.imagen.carga') }}",
    ...
    function cargar_lista_imagenes2(obj_dropzone, alias_examen)
    {
        ...
        $.each(temp, function( index, value )
        {
            ...
            if(value.status == "success")
            {
                ...
                {
                    if(value.dataURL.indexOf("http:") > -1)
                    {
                        // var img_temp = JSON.parse(value.xhr.response);
                        lista_imagenes[alias_examen][index] = [
                            url=value.dataURL,
                            nombre_origian= value.name,
                            nombre_img = value.name,

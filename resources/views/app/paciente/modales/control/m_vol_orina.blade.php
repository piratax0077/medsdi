        ...
        <div class="modal-content" >
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        <canvas id="grafico_vol_orina" height="100"></canvas>
                    </div>
                </div>
            </div>
        </div>
        ...
        .done(function(data) {
            ...
            if (data != null && data.estado == 1) {
                $.each(data.registros, function (key, value) {
                    ...
                    cantidades.push(parseFloat(value.lectura));
                    var html = '';
                    let partes = value.fecha.split(" ");
                    let soloFecha = partes[0];
                    let soloHora = partes[1];
                    console.log(soloHora);
                    html += '<tr>';
                    html += '    <td>'+soloFecha+'</td>';
                    html += '    <td>'+soloHora +'</td>';
            ...
            if (window.chartVolOrinaControl) {
                window.chartVolOrinaControl.destroy();
            }

            // Crea el gráfico solo si hay datos
            if (fechas.length > 0) {
                const ctx = document.getElementById('grafico_vol_orina').getContext('2d');
                window.chartVolOrinaControl = new Chart(ctx, {
                    type: 'line',
                    data: {
                        labels: fechas,
                        datasets: [
                            {
                                label: 'Volumen de Orina (ml)',
                                data: cantidades,
                    ...
                    options: {
                        ...
                        plugins: {
                            legend: { position: 'top' },
                            title: { display: true, text: 'Historial de Volumen de Orina' }
                        }
                    }
                });
            }

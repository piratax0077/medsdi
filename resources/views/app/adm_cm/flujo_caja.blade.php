                                                ...
                                                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 col-xxl-12 col-xxxl-12">
                                                    <div class="card">
                                                        ...
                                                        <div class="card-body">
                                                            <div class="table-responsive">
                                                                <div class="dt-responsive table-responsive">
                                                                    <table id="tabla_cajas" class="display table table-striped dt-responsive nowrap table-xs" style="width:100%">
                                                                        <thead>
                                                                            <tr>
                                                                                <th>Nombre</th>
                                                                                <th>Ubicación</th>
                                                                              ...
                                                                              @if(count($cajas) > 0)
                                                                                        @foreach($cajas as $caja)
                                                                                            <tr>
                                                                                                ...
                                                                                                <td>
                                                                                                    {{ $caja->nombre_lugar_atencion ?? 'N/A' }}
                                                                                                </td>

                                                                                                <td>
                                                                                                    <!-- editar caja -->
                                                                                                    <button class="btn btn-sm btn-warning btn-icon" onclick="editar_caja({{ $caja->id }});"><i class="fas fa-edit"></i></button>
                                                                                                    <!-- eliminar caja -->
                                                                                                    <button class="btn btn-sm btn-danger btn-icon" onclick="eliminar_caja({{ $caja->id }});"><i class="fas fa-trash"></i></button>
                                                                                                </td>
                                                                                            </tr>
                                                                                        @endforeach
                                                                                    @else
...
<div class="modal fade" id="editarCajaModal" tabindex="-1" role="dialog" aria-labelledby="editarCajaModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form id="formEditarCaja" method="POST" action="{{ route('flujo_caja.actualizar_caja') }}">
      @csrf
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="id_lugar_atencion" required>
                             ...
                             <div class="form-group">
                              ...
                              <div class="form-group">
                                <div class="col-12">
                                    ...
                                    <select class="form-control mt-2" id="institucion" required>
...
<script>
    ...
    $('#formEditarCaja').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                swal("¡Caja actualizada exitosamente!", {
                    icon: "success",
                }).then(() => {
                    let caja = data.caja;
                    console.log(caja);
                    // Actualizar datos en la tabla sin recargar la página
                    let row = $('#tabla_cajas tbody tr').filter(function() {
                        return $(this).find('td').eq(0).text() == caja.id;
                    });
                    row.find('td').eq(1).text(caja.nombre_caja);
                    row.find('td').eq(2).text(caja.ubicacion);
                    row.find('td').eq(3).text(caja.institucion);

                });
            },
            error: function(error) {
                console.log(error);
                swal("Error al actualizar caja", {
                    icon: "error",
                });
            }
        });
    ...
    function guardar_caja(){
        ...
        let id_lugar_atencion = $('#institucion').val();
        let ubicacion = $('#ubicacion_caja').val();
        ...
        let mensaje = "Corrija los siguientes errores:<br><br>";
            ...
            if(nombre_caja.trim() === ""){
                valido = 0;
                mensaje += "- El nombre de la caja es obligatorio.<br>";
            }

            if(id_lugar_atencion === "Seleccione Institución"){
                valido = 0;
                mensaje += "- Debe seleccionar una institución.<br>";
            }

            if(ubicacion.trim() === ""){
                valido = 0;
                mensaje += "- La ubicación de la caja es obligatoria.<br>";
            }
    ...
    function editar_caja(id) {
        $.ajax({
            url: '{{ route("flujo_caja.dame_caja") }}',
            type: 'GET',
            data: { id: id },
            success: function(response) {
                console.log(response);
                // Suponiendo que response trae los datos de la caja
                // Rellena los campos del modal de edición
                $('#editarCajaModal input[name="id"]').val(response.caja.id);
                $('#editarCajaModal input[name="nombre_caja"]').val(response.caja.nombre_caja);
                $('#editarCajaModal input[name="ubicacion"]').val(response.caja.ubicacion);
                $('#editarCajaModal select[name="id_lugar_atencion"]').val(response.caja.id_lugar_atencion);
    ...
    function eliminar_caja(id){
        swal({
            ...
            if (aceptar) {
                let url = "{{ route('flujo_caja.eliminar_caja') }}";
                $.ajax({
                    url: url,
                    type: 'post',
                    data: {
                    ...
                    success: function(data) {
                        console.log(data);
                        if(data.estado == 0){
                            swal("No se puede eliminar una caja abierta.", {
                                icon: "warning",
                            });
                        }else{
                            swal("¡Caja eliminada exitosamente!", {
                                icon: "success",
                            }).then(() => {
                                location.reload(); // Recargar la página para reflejar cambios
                            });

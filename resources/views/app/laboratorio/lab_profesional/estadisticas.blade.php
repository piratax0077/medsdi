             ...
             <div class="card">
                ...
                <div class="card-body">
                    <ul class="nav nav-tabs-secciones mb-2" id="estadisticas-aud" role="tablist">
                        ...
                        <li class="nav-item-secciones">
                            <a class="nav-secciones text-uppercase" id="estadisticas-remuneraciones-lab-tab" data-toggle="tab" href="#estadisticas-remuneraciones-lab" role="tab" aria-controls="estadisticas-remuneraciones-lab" aria-selected="false">Reportes Remuneraciones Laboratorio</a>
                        </li>
                    </ul>
                    <div class="tab-content" id="estadisticas-aud-content">
                            ...
                            <form id="form-filtro-estadisticas" class="mb-4">
                                <div class="form-row align-items-end">
                                    ...
                                    <div class="col-md-3">
                                     ...
                                     <div class="col-md-2">
                                        <label for="profesionales_estadisticas">Profesional:</label>
                                        <select class="form-control" id="profesionales_estadisticas" name="profesionales_estadisticas">
                        ...
                        <div class="tab-pane fade" id="estadisticas-remuneraciones-lab" role="tabpanel" aria-labelledby="estadisticas-remuneraciones-lab-tab">
                            <form id="form-filtro-estadisticas-remuneraciones" class="mb-4">
                                <div class="form-row align-items-end">
                                    <div class="col-md-2">
                                        <label for="fecha_inicio">Fecha inicio</label>
                                        <input type="date" class="form-control" id="fecha_inicio_remuneraciones" name="fecha_inicio_remuneraciones">
                                    </div>
                                    <div class="col-md-2">
                                        <label for="fecha_fin">Fecha fin</label>
                                        <input type="date" class="form-control" id="fecha_fin_remuneraciones" name="fecha_fin_remuneraciones">
                                    </div>
                                    <div class="col-md-3">
                                        <label for="sucursales">Sucursales</label>
                                        <select class="form-control" id="sucursales_remuneraciones" name="sucursales_remuneraciones">
                                            <option value="">Todas</option>
                                            @if(isset($sucursales))
                                            @foreach($sucursales as $sucursal)
                                                <option value="{{ $sucursal->id }}">{{ $sucursal->nombre }}</option>
                                    ...
                                    <div class="col-md-2">
                                        <label for="profesional_remuneraciones">Profesional:</label>
                                        <select class="form-control" id="profesional_remuneraciones" name="profesional_remuneraciones">
                                            <option value="">Todos</option>
                                            @if(isset($profesionales))
                                                @foreach($profesionales as $profesional)
                                                    <option value="{{ $profesional->id }}">
                            ...
                            <div id="resultados-estadisticas-remuneraciones">
                                <!-- Aquí se mostrarán los resultados -->
                            </div> 
                        </div>
                    </div>
...
$(document).ready(function() {
    $('#form-filtro-estadisticas').on('submit', function(e) {
        ...
        let id_profesional = $('#profesionales_estadisticas').val();
    ...
    $('#form-filtro-estadisticas-remuneraciones').on('submit', function(e) {
        e.preventDefault();
        let fecha_inicio = $('#fecha_inicio_remuneraciones').val();
        let fecha_fin = $('#fecha_fin_remuneraciones').val();
        let id_institucion = $('#id_institucion').val();
        let id_sucursal = $('#sucursales_remuneraciones').val();
        let id_profesional = $('#profesional_remuneraciones').val();
        ...
        // Mostrar loading
        $('#resultados-estadisticas-remuneraciones').html('<div class="text-center py-4"><span class="spinner-border"></span> Buscando...</div>');

        // Petición AJAX (ajusta la ruta según tu backend)
        $.ajax({
            url: '{{ route("laboratorio.estadisticas.buscar_remuneraciones") }}',
            type: 'GET',
            data: {
                fecha_inicio: fecha_inicio,
                fecha_fin: fecha_fin,
            ...
            success: function(response) {
                console.log(response);
                // Aquí puedes renderizar los resultados
                $('#resultados-estadisticas-remuneraciones').html(response || '<div class="alert alert-info">No hay resultados para el rango seleccionado.</div>');
            },
            error: function(error) {
                console.log(error);
                $('#resultados-estadisticas-remuneraciones').html('<div class="alert alert-danger">Error al buscar estadísticas.</div>');
            }
        });
    });
});

...
(function() {
    ...
    tratamientos.forEach(t => {
        ...
        if (!tratamientosData[nombre]) {
            tratamientosData[nombre] = { total: 0, color: colores[colorIndex % colores.length] };
            colorIndex++;
        }
        tratamientosData[nombre].total += t.cantidad;
    });
    let tratLabels = Object.keys(tratamientosData);
    let tratData = tratLabels.map(n => tratamientosData[n].total);
    ...
    tratLabels.forEach((nombre, i) => {
        leyendaHtml += `<li><span style="display:inline-block;width:20px;height:20px;background:${tratColors[i]};margin-right:5px;"></span>${nombre}</li>`;
    });
    leyendaHtml += '</ul>';

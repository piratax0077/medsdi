@foreach ($cuerpo['detalle_receta'] as $detalle)
    <!DOCTYPE html>
    <html lang="es">
    ...
    <body>
        <div class="contenido">
            ...
            @foreach ($detalle as $medicamento)
                ...
                <table style="width:100%">
                    <tr>
                        <td><strong>{{ $medicamento['nombre_medicamento'] }}</strong> {{ $medicamento['droga'] }}</td>
                        <td>{{ $medicamento['presentacion'] }}</td>
                        <td style="text-align: right;">{{ $medicamento['cantidad_compra'] }}</td>
                    </tr>
                    <tr>
                        <td colspan="2" style="padding-left: 0.7rem;"><strong >Tr: </strong>{{ $medicamento['posologia'] }}, {{ $medicamento['via_administracion'] }}, {{ $medicamento['periodo'] }}</td>
                        <td style="text-align: right;">{{ ($medicamento['uso_cronico']==1?"USO CRONICO":"") }}</td>
                    </tr>
                </table>
                <br>
            @endforeach

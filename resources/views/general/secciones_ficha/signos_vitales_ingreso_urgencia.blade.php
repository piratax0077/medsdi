...
<div class="form-row">
    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
        ...
        <div class="d-inline btn-group btn-group float-md-right" role="group" aria-label="First group">
...
<div id="contenedor_evoluciones_paciente" class="d-none">
        ...
        @foreach ($controles_ciclo as $cc)
            <div class="col-sm-12 pb-0 mb-2 d-inline">
                <p class="pt-3 d-inline">
                   <i class="feather icon-user text-c-blue"></i> {{ $cc->created_at->format('d/m/Y H:i') }}  <i class="feather icon-clock text-c-blue"></i>{{ $cc->nombre }}
                </p>
            </div>
            ...
            <div class="card-lineal">
                <div class="card-header-lineal">
                    <i class="feather icon-user text-c-blue"></i> {{ $cc->created_at->format('d/m/Y H:i') }} {{ $cc->nombre }}
                </div>
                ...
                <div class="form-row">
                    <div class="col-sm-12 col-md-3 col-lg-3 col-xl-3">
                        <div class="form-group">
                            <label class="floating-label-activo-sm">Tipo de Respiración</label>
...
<!--Tabla-->
    ...
    <h5 class="text-dark f-18 mt-3">Evoluciones</h5>

    <div class="table-responsive mb-3">
        <table id="tabla_cont_ciclo" class="table table-bordered table-sm tabla_cont_ciclo w-100">
            <thead>
                <tr>
                    <th class="text-center align-middle" style="display:none">id</th>
                    {{-- <th class="text-center align-middle" style="display:none">id-resp</th> --}}
                    <th class="align-middle">Fecha/hora</th>
                    <th class="align-middle">T°</th>
                    <th class="align-middle">Pulso</th>
                    <th class="align-middle">PAS</th>
                    <th class="align-middle">PAD</th>
                    <th class="align-middle">PAM</th>
                    <th class="align-middle">FR</th>
            ...
            <tbody>
                @if(count($controles_ciclo) > 0)
                    @foreach ($controles_ciclo as $cc)
                        <tr>
                            <td style="display:none">{{ $cc->id }}</td>
                            {{-- <td style="display:none">{{ $cc->datos_evolucion->id }}</td> --}}
                            <td class="align-middle">{{ $cc->created_at->format('d/m/Y H:i') }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->temperatura }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->pulso }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->pas }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->pad }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->pam }}</td>
                            <td class="align-middle">{{ $cc->datos_evolucion->fr }}</td>
                            <td class="align-middle">
                                HGT: {{ $cc->datos_evolucion->hemoglucotest }}<br>
                                Glasgow: {{ $cc->datos_evolucion->glasgow }}<br>
                                EVA: {{ $cc->datos_evolucion->eva }}<br>
                                Otras: {{ $cc->datos_evolucion->otras_pruebas }}
                            </td>
                            <td class="text-center align-middle">{{ $cc->datos_evolucion->gravedad }}</td>
                            <td class="text-center align-middle">
                                @if($enfermera)
                                <button type="button" class="btn btn-icon btn-danger-light-c"
                                    onclick="eliminarEvolucionAgregada({{ $cc->id }})"><i class="feather icon-x"></i>
                                </button>
                                @endif
                            </td>
                        </tr>
                ...
                @else
                    <tr>
                        <td colspan="11" class="text-center align-middle">No hay controles de ciclo registrados</td>
                    </tr>
                @endif
            </tbody>
        </table>

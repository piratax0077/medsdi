                                                            ...
                                                            <div class="table-responsive">
                                                                <table id="table_piezas_presupuesto_odonto" class="display table table-striped dt-responsive nowrap table-sm dataTable no-footer dtr-inline w-100 mt-2">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @if($o->urgencia == 0 <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
    <div class="card-a">
        <div class="card-header-a" id="exam_esp">
            <button class="accor-closed btn pt-1 pb-0 pl-1 btn-block text-left card-act-open collapsed" type="button" data-toggle="collapse" data-target="#exam_esp_c" aria-expanded="false" aria-controls="exam_esp_c">
                Examen Odontológico General
                                                                        ...
                                                                        @foreach ($todos as $grupo)
                                                                            @if($grupo->presupuesto == 1)
                                                                                <tr>
                                                                                    ...
                                                                                    <td>
                                                                                        <button type="button" class="btn btn-danger btn-icon" onclick="eliminar_diagnostico({{ $grupo->id }},'gral')"><i class="feather icon-x"></i></button>
                                                                                    </td>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const seleccionInicial = new Set(
            odontograma.filter(item => item.presupuesto == 1 && item.urgencia == 0).map(item => item.pieza)
        );
        (grupos_odontograma_global || []).forEach(function(grupo) {
            ...
            if (localizacion.includes('maxilar superior') || localizacion.includes('boca completa')) {
                ['1','2'].forEach(c => { for (let i = 1; i <= 8; i++) seleccionInicial.add(c + '.' + i); });
            ...
            if (localizacion.includes('maxilar inferior') || localizacion.includes('boca completa')) {
                ['3','4'].forEach(c => { for (let i = 1; i <= 8; i++) seleccionInicial.add(c + '.' + i); });
                document.getElementById('max_inf').checked = true;
            }
        });
        const piezasUnicas = [...seleccionInicial];
...
<script>
    ...
    function seleccionar_maxilar_superior() {
        ...
        // Un maxilar es una selección exclusiva: no conservar piezas del
        // maxilar contrario que hayan quedado seleccionadas anteriormente.

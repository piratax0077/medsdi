<nav class="pcoded-navbar menu-light navbar-collapsed">
    <div class="navbar-wrapper">
        <div class="navbar-content scroll-div">
            <div class="">
                <div class="main-menu-header">
                    <img class="img-radius img-fluid wid-100" id="profile-image"
                                                        src="{{ $asistente->foto_perfil ? asset('storage/' . $asistente->foto_perfil) : asset('images/iconos/usuario_asistente.svg') }}"
                                                        alt="User image">
                    <div class="user-details">
                        <div id="more-details">{{ @Auth::user()->name }}
							<i class="fa fa-caret-down"></i>
						</div>
                    </div>
                </div>
                <div id="nav-user-link">
                    <ul class="list-inline">
                        <li class="list-inline-item">
                            <a href="{{ ROUTE('asistentejcm.perfil') }}" data-toggle="tooltip" title="Mi perfil">
                                <i class="feather icon-user"></i>
                            </a>
                        </li>
                        <li class="list-inline-item">
                            <form id="close" action="{{ ROUTE('logout') }}" method="POST">
                                @csrf
                                <a href="javascript:{}" onclick="document.getElementById('close').submit();"
                                    data-toggle="tooltip" title="Cerrar sesión" class="text-danger">
                                    <i class="feather icon-power"></i>
                                </a>
                            </form>
                        </li>
                    </ul>
                </div>
            </div>
            <ul class="nav pcoded-inner-navbar ">
                <li class="nav-item pcoded-hasmenu">
                    <a href="javascript:void(0)" class="nav-link">
						<span class="pcoded-micon">
							<i class="feather icon-home"></i>
						</span>
						<span class="pcoded-mtext text-center">Mi Escritorio</span></a>
                    <ul class="pcoded-submenu">
                        <li>
                            <a href="{{ ROUTE('asistentejcm.home') }}">Mi Escritorio</a>
                        </li>
                        <li>
                            <a href="{{ ROUTE('asistentejcm.buscar_paciente') }}">Buscar Pacientes</a>
                        </li>
                        {{--  <li>
                            <a href="{{ ROUTE('asistentejcm.reservar_hora') }}">Reservar Hora Médica</a>
                        </li>  --}}
                        <li>
                            <a href="{{ ROUTE('asistentejcm.mis_profesionales') }}">Profesionales</a>
                        </li>
                        <li>
                            <a href="{{ ROUTE('asistentejcm.rendir') }}">Manejo de Caja</a>
                        </li>
                    </ul>
                </li>
                <li class="nav-item pcoded-hasmenu">
                    <a href="javascript:void(0)" class="nav-link">
                        <span class="pcoded-micon">
							<i class="feather feather icon-menu"></i>
						</span>
						<span class="pcoded-mtext text-center">Otras opciones</span>
					</a>
                    <ul class="pcoded-submenu">
                        <li>
							<a href="{{ ROUTE('asistentejcm.perfil') }}">Editar Perfil</a>
						</li>

                    </ul>
                </li>
            </ul>
        </div>
    </div>
</nav>


...

{
    ...
    public function registrar_odontograma(Request $request)
    {
        try {
            ...
            if (!$profesional || !preg_match('/^[1-8]\.[1-8]$/', (string) $request->pieza)) {
                return ['status' => 0, 'mensaje' => 'Profesional o número de pieza inválido.'];
            }
            ...
            $odontograma = null;
            if ($request->id_presupuesto) {
                $odontograma = OdontogramaPaciente::when($request->id_odontograma, function ($query, $idOdontograma) {
                        return $query->where('id', $idOdontograma);
                    })
            ...
            if ($request->id_odontograma && !$odontograma) {
                return ['status' => 0, 'mensaje' => 'El tratamiento seleccionado no pertenece a este presupuesto. Recargue la ficha e intente nuevamente.'];
            }
            $esNuevoRegistro = !$odontograma;
            if ($presupuestoSeleccionado && (int) $presupuestoSeleccionado->estado !== 1 && $esNuevoRegistro) {
                return ['status' => 0, 'mensaje' => 'El presupuesto está cerrado. Solo puede actualizar las caras de piezas que ya pertenecen a él.'];
            }
            ...
            $odontograma->fecha = Carbon::now()->format('Y-m-d H:i:s');
            if ($esNuevoRegistro) {
                $odontograma->estado = 0;
                $odontograma->urgencia = 0;
            }
            ...
            $esPiezaPediatrica = preg_match('/^[5-8]\.[1-5]$/', (string) $request->pieza) === 1;
            $vistaOdontograma = $esPiezaPediatrica
                ? 'atencion_odontologica.generales.odontograma_pediatrico_completo'
                : 'atencion_odontologica.generales.odontograma_adulto';
            $odontograma_paciente_vista = view($vistaOdontograma, ['odontograma_historial' => $odontograma_paciente_historial])->render();
            $valores = $this->dameValoresOdontograma($request->id_paciente, $request->id_ficha_atencion, $request->id_lugar_atencion, $profesional->id_tipo_especialidad, $request->id_presupuesto);
    ...
    public function dameValoresOdontograma($id_paciente, $id_ficha_atencion, $id_lugar_atencion, $id_tipo_especialidad = null, $id_presupuesto = null){
        $fichaController = new ficha_atencionController;
        $valores = $fichaController->dameValores($id_paciente, $id_ficha_atencion, $id_lugar_atencion, $id_tipo_especialidad, $id_presupuesto);
        return $valores;
    }

    public function dame_odontograma_paciente($id_paciente, $id_ficha_atencion, $id_lugar_atencion, $tipo_especialidad, $id_presupuesto = null, $id_profesional = null){
        $fichaController = new ficha_atencionController;
        $odontograma_paciente = $fichaController->dameOdontogramaPaciente($id_paciente, $id_ficha_atencion, $id_lugar_atencion, $tipo_especialidad, $id_presupuesto, $id_profesional);
    ...
    public function cargar_tratamiento_presupuesto(Request $request){
        ...
        if($request->tipo == null){
            if (!$profesional) {
                return ['status' => 0, 'mensaje' => 'No fue posible identificar al profesional.'];
            }

            $pieza = OdontogramaPaciente::whereKey($request->id)
                ->where('id_profesional', $profesional->id)
                ->first();
            if (!$pieza) {
                return ['status' => 0, 'mensaje' => 'La pieza seleccionada no pertenece a este profesional.'];
            }

            // No se modifica la pieza hasta que el profesional confirme la reapertura.
            $presupuesto = PresupuestosDental::where('id_paciente', $pieza->id_paciente)
                ->where('id_profesional', $profesional->id)
                ->where('id_lugar_atencion', $pieza->id_lugar_atencion)
                ->where('id_ficha_atencion', $pieza->id_ficha_atencion)
                ->first();
            $presupuestoCerrado = $presupuesto
                && ((int) $presupuesto->estado !== 1 || (bool) $presupuesto->pago_completado);

            if ($presupuestoCerrado && !$request->boolean('reabrir_presupuesto')) {
                return [
                    'status' => 2,
                    'requiere_reapertura' => true,
                    'mensaje' => 'El presupuesto está cerrado. Para agregar la pieza '.$pieza->pieza.' debe reabrirlo y recalcular su saldo.',
                ];
            }

            $presupuestoReabierto = false;
            if ($presupuestoCerrado) {
                $presupuesto->estado = 1;
                $presupuesto->pago_completado = false;
                $presupuesto->fecha_pago_completo = null;
                $presupuesto->save();
                $presupuestoReabierto = true;
            }
            ...
            if($pieza->save()){
                ...
                if(!$presupuesto){
                    ...
                    $presupuesto->datos_piezas_dentales = json_encode([]);
                    $presupuesto->estado = 1;
                ...
                $pieza->save();
                InsumosTratamientosDental::where('id_tratamiento', $pieza->id)
                    ->where('tipo', 'pack_automatico')
                    ->update([
                        'presupuesto' => 1,
                ...
                $valor_total = intval($valores[0]) + intval($valores[1]) + intval($valores[2]) + intval($valores[3] ?? 0);
                $presupuesto->valor_total = $valor_total;
                $totalAbonadoPresupuesto = (int) PagosPresupuestoDental::where('id_presupuesto', $presupuesto->id)->sum('total');
                $presupuesto->valor_abonado = $totalAbonadoPresupuesto;
                $presupuesto->pago_completado = $valor_total > 0 && $totalAbonadoPresupuesto >= $valor_total;
                $presupuesto->fecha_pago_completo = $presupuesto->pago_completado
                    ? ($presupuesto->fecha_pago_completo ?: Carbon::now())
                    : null;
                $presupuesto->save();
                $this->reconciliarAsignacionPresupuestoDental($presupuesto, $profesional);
                $odontograma_paciente = $this->dame_odontograma_paciente(
                    $pieza->id_paciente,
                    $pieza->id_ficha_atencion,
                    $pieza->id_lugar_atencion,
                ...
                // El bloque anterior se conserva por compatibilidad de cálculo, pero la
                // respuesta debe reflejar los estados autoritativos recién conciliados.
                $odontograma_paciente = $this->dame_odontograma_paciente(
                    $pieza->id_paciente,
                    $pieza->id_ficha_atencion,
                    $pieza->id_lugar_atencion,
                ...
                return [
                    'status' => 1,
                    'mensaje' => $presupuestoReabierto
                        ? 'El presupuesto fue reabierto y la pieza '.$pieza->pieza.' se agregó correctamente.'
                        : 'Pieza '.$pieza->pieza.' agregada con éxito.',
                    'odontograma_paciente' => $odontograma_paciente,
                    'valores' => $valores,
                    'presupuesto' => $presupuesto,
                    'presupuesto_reabierto' => $presupuestoReabierto,
                    'presupuesto_completado' => (bool) $presupuesto->pago_completado,
                    'total_abonado' => $totalAbonadoPresupuesto,
                    'saldo_pendiente' => max(0, $valor_total - $totalAbonadoPresupuesto),
                ];
            }else{
            ...
            if($pieza->save()){
                ...
                // Los grupos dentales pertenecen al presupuesto clínico vigente,
                // de la misma forma que las prestaciones registradas por pieza.
                $pieza->id_presupuesto = $presupuesto->id;
                $pieza->save();
                ...
                return [
                    'status' => 1,
                    'mensaje' => 'Grupo '.$pieza->pieza.' agregado al presupuesto con éxito.',
                    'valores' => $valores,
            ...
            $presupuesto = PresupuestosDental::where('id_paciente', $request->id_paciente)->where('id_lugar_atencion', $request->id_lugar_atencion)->where('id_ficha_atencion', $request->id_ficha_atencion)->first();
                ...
                // Al cambiar el total del presupuesto, redistribuir todos los abonos
                // con la misma regla utilizada por Pagos y estados. Esto evita que
                // la tabla y el odontograma conserven colores calculados por rutas distintas.
                $this->reconciliarAsignacionPresupuestoDental($presupuesto, $profesional);
                $presupuesto->refresh();
                $odontograma_paciente = $this->dame_odontograma_paciente(
                    $request->id_paciente,
        ...
        try {
            ...
            foreach ($valores as $v) {
                if ($v['info'] == 'odonto') {
                    ...
                    $valor = min(intval($pieza->valor), max(0, intval($v['valor'] ?? $pieza->valor)));
    ...
    public function cargar_tratamiento_presupuesto_period(Request $request){
        ...
        if (!$profesional) {
            return response()->json(['status' => 0, 'mensaje' => 'No se encontró el profesional autenticado.'], 403);
        }
        ...
        $fichaValida = FichaAtencion::where('id', $request->id_ficha_atencion)
            ...
            ->exists();
        if (!$fichaValida) {
            return response()->json(['status' => 0, 'mensaje' => 'La ficha no pertenece al paciente seleccionado.'], 422);
        }

        if ($request->id_presupuesto) {
            $presupuestoValido = PresupuestosDental::where('id', $request->id_presupuesto)
                ->where('id_paciente', $request->id_paciente)
                ->where('id_ficha_atencion', $request->id_ficha_atencion)
                ->where('id_profesional', $profesional->id)
                ->exists();
            if (!$presupuestoValido) {
                return response()->json(['status' => 0, 'mensaje' => 'El presupuesto no pertenece a esta ficha o profesional.'], 422);
            }
        }

        if ($request->piezas) {
            $patronPieza = $request->tipo === 'odped' ? '/^[5-8]\.[1-5]$/' : '/^[1-4]\.[1-8]$/';
            $piezas = array_values(array_unique($request->piezas));
            foreach ($piezas as $pieza) {
                if (!preg_match($patronPieza, $pieza)) {
                    return response()->json(['status' => 0, 'mensaje' => "La pieza {$pieza} no corresponde al odontograma seleccionado."], 422);
                }
            }
            $request->merge(['piezas' => $piezas]);
        }
        ...
        if(!$request->piezas){
            ...
            if ($odontograma->save()) {
                // crear el presupuesto si es que aun no se ha registrado
                $consultaPresupuestoActivo = PresupuestosDental::query()
                    ->where('id_paciente', $odontograma->id_paciente)
                    ->where('id_profesional', $profesional->id)
                    ->where('id_lugar_atencion', $odontograma->id_lugar_atencion)
                    ->where('id_ficha_atencion', $odontograma->id_ficha_atencion)
                    ->where('estado', 1);
                $presupuesto = $request->id_presupuesto
                    ? (clone $consultaPresupuestoActivo)->where('id', $request->id_presupuesto)->first()
                    : null;
                $presupuesto = $presupuesto ?: $consultaPresupuestoActivo->orderByDesc('id')->first();
                if(!$presupuesto){
                    $presupuesto = new PresupuestosDental;
                ...
                $odontograma_paciente_vista = view('atencion_odontologica.generales.odontograma_adulto',['odontograma_historial' => $odontograma_paciente])->render();
                $valores = $this->dameValoresOdontograma($odontograma->id_paciente, $odontograma->id_ficha_atencion, $odontograma->id_lugar_atencion, $profesional->id_tipo_especialidad, $presupuesto->id);
                $insumos = (new ficha_atencionController())->dame_insumos_tratamiento($odontograma->id_paciente, $odontograma->id_ficha_atencion)
                    ->where('id_presupuesto', $presupuesto->id)
                    ->values();
                ...
                // Una pieza agregada después de un pago cambia el total del presupuesto.
                // Redistribuimos todo lo abonado contra el contenido vigente para no
                // ofrecer como "remanente" un pago que ya cubrió otra prestación.
                $this->reconciliarAsignacionPresupuestoDental($presupuesto, $profesional);
                ...
                // El bloque anterior conserva cálculos de presentación heredados.
                // Recargamos los estados persistidos por la conciliación antes de responder.
                $odontograma_paciente = $this->dame_odontograma_paciente(
                    $odontograma->id_paciente,
                    $odontograma->id_ficha_atencion,
                    $odontograma->id_lugar_atencion,
                    $profesional->id_tipo_especialidad
                );
            ...
            foreach($piezas as $pieza){
                $consultaOdontograma = OdontogramaPaciente::where('id_paciente', $request->id_paciente)
                    ->where('id_profesional', $profesional->id)
                    ->where('id_ficha_atencion', $request->id_ficha_atencion)
                    ->where('id_lugar_atencion', $request->id_lugar_atencion)
                    ->where('pieza', $pieza)
                    ->where('urgencia', $request->urgencia ? 1 : 0);

                if ($request->etapa === 'diagnostico') {
                    // En el examen oral existe un solo diagn&oacute;stico pendiente por
                    // pieza y presupuesto; a&uacute;n no se exige tratamiento.
                    $consultaOdontograma->where(function ($query) {
                        $query->whereNull('tratamiento')->orWhere('tratamiento', '');
                    });
                } elseif ($request->etapa === 'planificacion') {
                    // Completa el mismo registro creado en el examen oral, evitando
                    // duplicar la pieza al asignarle posteriormente un tratamiento.
                    $consultaOdontograma->where(function ($query) {
                        $query->whereNull('tratamiento')->orWhere('tratamiento', '');
                    });
                    if ($request->id_presupuesto) {
                ...
                } elseif ($request->etapa === 'completa') {
                    // Si un intento anterior alcanz&oacute; a guardar tratamiento y
                    // caras pero perdi&oacute; el diagn&oacute;stico, completar esa misma
                    // fila al reintentar en vez de crear una pieza duplicada.
                    $consultaOdontograma
                        ->where('tratamiento', $request->tto)
                ...
                $odontograma = $consultaOdontograma->orderByDesc('id')->first();
                $esNuevaPieza = !$odontograma;
                if ($esNuevaPieza) {
                    $odontograma = new OdontogramaPaciente();
                }
                ...
                if ($request->etapa !== 'planificacion') {
                    $odontograma->diagnostico = $request->diagnostico;
                }
            ...
            $insumos = (new ficha_atencionController())->dame_insumos_tratamiento($odontograma->id_paciente, $odontograma->id_ficha_atencion)
                ->where('id_presupuesto', $presupuesto->id)
                ->values();
            return ['status' => 1, 'mensaje' => 'Piezas agregada con éxito.', 'odontograma_paciente' => $odontograma_paciente, 'valores' => $valores,'presupuesto' => $presupuesto,'insumos' => $insumos,'vista_presupuestos' => $vista_presupuestos,'odontograma_paciente_vista' => $odontograma_paciente_vista];
        }
    ...
    private function agregarPackInsumosTratamiento(OdontogramaPaciente $odontograma, Profesional $profesional): void
    {
        ...
        // Implantologia utiliza su propio catalogo. Antes solo se consultaba
        // diagnosticos_dental, por lo que tratamientos como "Implante inmediato"
        // nunca encontraban el arancel (ni, por tanto, su pack de insumos).
        $descripcion = trim($odontograma->tratamiento);
        $idsCatalogo = collect([
            DiagnosticosDental::whereRaw('TRIM(UPPER(descripcion)) = ?', [mb_strtoupper($descripcion, 'UTF-8')])->value('id'),
            TratamientosImplantologia::whereRaw('TRIM(UPPER(descripcion)) = ?', [mb_strtoupper($descripcion, 'UTF-8')])->value('id'),
        ])->filter()->unique()->values();

        if ($idsCatalogo->isEmpty()) return;

        $arancel = DiagnosticosDentalProfesional::where('id_profesional', $profesional->id)
            ->whereIn('id_diagnostico', $idsCatalogo)
            ->whereExists(function ($query) {
                $query->selectRaw('1')
                    ->from('tratamientos_profesional_insumos as pack_configurado')
                    ->whereColumn('pack_configurado.id_diagnostico_profesional', 'diagnosticos_dental_profesional.id')
                    ->where('pack_configurado.estado', 1);
            })
            ->first();
        if (!$arancel) return;
    ...
    public function sacar_tratamiento_presupuesto(Request $request){
        ...
        if($request->tipo == null){
            ...
            if (!$pieza) {
                return ['status' => 0, 'mensaje' => 'No se encontró la pieza seleccionada.'];
            }
            $idPresupuestoOrigen = $pieza->id_presupuesto;
            $pieza->presupuesto = 0;
            if($pieza->save()){
                // Conservar el presupuesto de origen antes de desligar la pieza.
                // Buscar el primero de la ficha mezclaba presupuestos históricos.
                $presupuesto = $idPresupuestoOrigen
                    ? PresupuestosDental::find($idPresupuestoOrigen)
                    : PresupuestosDental::where('id_paciente', $pieza->id_paciente)
                        ->where('id_lugar_atencion', $pieza->id_lugar_atencion)
                        ->where('id_ficha_atencion', $pieza->id_ficha_atencion)
                        ->where('estado', 1)
                        ->orderByDesc('id')
                ...
                $odontograma_paciente = $this->dame_odontograma_paciente(
                    ...
                    $pieza->id_lugar_atencion,
                    $profesional->id_tipo_especialidad,
                    optional($presupuesto)->id
                )->where('presupuesto', 1)->where('urgencia', 0)->values();
                $odontograma_paciente_vista = view('atencion_odontologica.generales.odontograma_adulto',['odontograma_historial' => $odontograma_paciente])->render();
                $valores = $this->dameValoresOdontograma($pieza->id_paciente, $pieza->id_ficha_atencion, $pieza->id_lugar_atencion, $profesional->id_tipo_especialidad, optional($presupuesto)->id);
                $valor_total = $valores[0] + $valores[1] + $valores[2] + $valores[3];
                if ($presupuesto) {
                    $presupuesto->valor_total = $valor_total;
                }
                if($presupuesto && $presupuesto->valor_total == 0){
                    //$presupuesto->estado = 0;
                }
                if ($presupuesto) {
                    $presupuesto->save();
                }
                $pagos_tratamientos_dentales = PagosPresupuestoDental::where('id_presupuesto', optional($presupuesto)->id)->get();
            ...
            $pieza = ExamenesBocaGeneral::find($request->id);
            $idPresupuestoGrupo = $pieza->id_presupuesto;
            $pieza->presupuesto = 0;
            $pieza->id_presupuesto = null;
            if($pieza->save()){
                // crear el presupuesto si es que aun no se ha registrado
                $presupuesto = $idPresupuestoGrupo
                    ? PresupuestosDental::find($idPresupuestoGrupo)
                    : PresupuestosDental::where('id_paciente', $pieza->id_paciente)
                        ->where('id_lugar_atencion', $pieza->id_lugar_atencion)
                        ->where('id_ficha_atencion', $pieza->id_ficha_atencion)
                        ->where('estado', 1)
                ...
                return [
                    'status' => 1,
                    'mensaje' => 'Grupo '.$pieza->pieza.' sacado del presupuesto con éxito.',
                    'valores' => $valores,
                    'maxilar_superior_gral_tratamientos' => $maxilar_superior_gral_tratamiento,
                    'maxilar_superior_gral_diagnosticos' => $maxilar_superior_gral_diagnostico,
                    'maxilar_inferior_gral_tratamientos' => $maxilar_inferior_gral_tratamiento,
    ...
    public function dameTratamientosBocaGeneral($id_ficha_atencion, $total = null, $id_profesional = null)
    {
    ...
    public function agregar_insumos_tratamiento(Request $req){
        ...
        if ($req->filled('idProducto')) {
            $productoCatalogo = Producto::where('estado', 1)->find($req->idProducto);
        }
        if (!$productoCatalogo) {
            $productoCatalogo = Producto::where('estado', 1)
                ->where('nombre', $req->insumos)
                ->when((int) $req->idTipoInsumo === 1 && $req->idMarcaInsumo, function ($query) use ($req) {
                    $query->where('id_marca_implante', $req->idMarcaInsumo);
                })
                ->where(function ($query) use ($profesional) {
                    $query->whereNull('id_profesional')->orWhere('id_profesional', $profesional->id);
                })
                ->first();
        }
        ...
        $insumo->id_producto = $productoCatalogo ? $productoCatalogo->id : null;
    ...
    private function reconciliarAsignacionPresupuestoDental(PresupuestosDental $presupuesto, Profesional $profesional): int
    {
        $porcentaje = 0;
        if ($presupuesto->id_convenio_aplicado) {
            $convenio = EmpresasConvenios::find($presupuesto->id_convenio_aplicado);
            $porcentaje = $convenio ? intval($convenio->porcentaje) : 0;
        }
        $valorNeto = function ($valor) use ($porcentaje) {
            $valor = intval($valor);
            return $porcentaje > 0 ? (int) round($valor - ($valor * $porcentaje / 100)) : $valor;
        };

        $totalPagado = (int) PagosPresupuestoDental::where('id_presupuesto', $presupuesto->id)->sum('total');
        $saldo = max(0, $totalPagado);
        $totalPresupuesto = 0;

        $insumos = InsumosTratamientosDental::where('id_ficha_atencion', $presupuesto->id_ficha_atencion)
            ...
            ->get()->sortBy(function ($insumo) use ($valorNeto) {
                return $valorNeto(intval($insumo->valor) * max(1, intval($insumo->cantidad)));
            });

        // El valor de la pieza proviene del catálogo de tratamientos y no de una
        // columna física del odontograma. Usar la consulta enriquecida evita
        // conciliar las piezas como si costaran $0.
        $piezas = $this->dame_odontograma_paciente(
            ...
            $presupuesto->id,
            $profesional->id
        )->filter(function ($pieza) use ($presupuesto) {
            return (int) $pieza->id_presupuesto === (int) $presupuesto->id
                && (int) $pieza->presupuesto === 1
                && (int) ($pieza->urgencia ?? 0) === 0;
        })->sortBy(function ($pieza) use ($valorNeto) {
                return sprintf('%012d-%012d', $valorNeto($pieza->valor), intval($pieza->id));
            });

        $grupos = $this->dameTratamientosBocaGeneral($presupuesto->id_ficha_atencion, null, $profesional->id)
            ->filter(function ($grupo) use ($presupuesto) {
                return (int) $grupo->presupuesto === 1
                    && (int) $grupo->id_presupuesto === (int) $presupuesto->id;
            })
            ->sortBy(function ($grupo) use ($valorNeto) {
                return sprintf('%012d-%012d', $valorNeto($grupo->valor), intval($grupo->id));
            });

        $asignarEstado = function ($modelo, int $monto, string $tipo) use (&$saldo, &$totalPresupuesto) {
            ...
            if ($tipo === 'pieza') {
                OdontogramaPaciente::whereKey($modelo->id)->update(['estado_pago' => $estado]);
            } elseif ($tipo === 'insumo') {
                InsumosTratamientosDental::whereKey($modelo->id)->update(['estado_pago' => $estado]);
            } else {
                ExamenesBocaGeneral::whereKey($modelo->id)->update(['estado_pago' => $estado]);
            }
        };

        // Prioridad clínica de distribución: primero insumos y luego piezas,
        // ordenando cada categoría desde el menor valor neto al mayor.
        foreach ($insumos as $insumo) {
            $asignarEstado($insumo, $valorNeto(intval($insumo->valor) * max(1, intval($insumo->cantidad))), 'insumo');
        }
        foreach ($piezas as $pieza) {
            $asignarEstado($pieza, $valorNeto($pieza->valor), 'pieza');
        }
        foreach ($grupos as $grupo) {
            $asignarEstado($grupo, $valorNeto($grupo->valor), 'grupo');
        }

        $presupuesto->valor_total = $totalPresupuesto;
        $presupuesto->valor_abonado = $totalPagado;
        $presupuesto->pago_completado = $totalPresupuesto > 0 && $totalPagado >= $totalPresupuesto;
        $presupuesto->fecha_pago_completo = $presupuesto->pago_completado
            ? ($presupuesto->fecha_pago_completo ?: Carbon::now())
            : null;
        $presupuesto->save();
    ...
    public function dame_bono_pago(Request $req){
        ...
        $remanenteConciliado = null;
        $profesionalConciliacion = Profesional::where('id_usuario', Auth::user()->id)->first();
        if ($presupuesto && $profesionalConciliacion
            && (int) $presupuesto->id_profesional === (int) $profesionalConciliacion->id
            && (int) $presupuesto->id_paciente === (int) $req->id_paciente) {
            $remanenteConciliado = $this->reconciliarAsignacionPresupuestoDental($presupuesto, $profesionalConciliacion);
        }
        ...
        // La conciliación sobre el total histórico es la fuente de verdad. El flag
        // `asignado` se mantiene por compatibilidad con pagos antiguos, pero no debe
        // volver a habilitar dinero que ya fue consumido por una prestación.
        if ($remanenteConciliado !== null) {
            $nuevoAbono = $remanenteConciliado;
            $montoDisponibleReasignar = $remanenteConciliado;
        }
        ...
        // Invariante de pago: sin abonos reales no puede existir ninguna
        // prestación cubierta. Además de corregir registros antiguos, se
        // actualizan los modelos que se devolverán al modal en esta respuesta.
        if ($totalPagado <= 0) {
            foreach ($odontogramaActual as $pieza) {
                if ((int) $pieza->presupuesto !== 1 || (int) ($pieza->urgencia ?? 0) === 1) {
                    continue;
                }
                $pieza->estado_pago = 'error';
                OdontogramaPaciente::whereKey($pieza->id)->update(['estado_pago' => 'error']);
            }
            foreach ($insumosActuales as $insumo) {
                if ((int) $insumo->id_presupuesto !== (int) $presupuesto->id
                    || (int) $insumo->presupuesto !== 1
                    || (int) ($insumo->urgencia ?? 0) === 1) {
                    continue;
                }
                $insumo->estado_pago = 'error';
                InsumosTratamientosDental::whereKey($insumo->id)->update(['estado_pago' => 'error']);
            }
            foreach ($todosActuales as $grupo) {
                if ((int) ($grupo->id_presupuesto ?? 0) !== (int) $presupuesto->id
                    || (int) $grupo->presupuesto !== 1) {
                    continue;
                }
                $grupo->estado_pago = 'error';
                ExamenesBocaGeneral::whereKey($grupo->id)->update(['estado_pago' => 'error']);
            }
        }
    ...
    public function pago_presupuesto(Request $req){
        ...
        if ((bool) $presupuesto->pago_completado) {
            return ['estado' => 0, 'presupuesto_completado' => true, 'mensaje' => 'Este presupuesto ya está pagado y cerrado. No admite nuevos abonos.'];
        }
        if (!is_numeric($pago_actual) || intval($pago_actual) <= 0) {
            return ['estado' => 0, 'mensaje' => 'El monto del pago debe ser mayor que cero.'];
        }

        $abonadoAntesDelPago = (int) PagosPresupuestoDental::where('id_presupuesto', $presupuesto->id)->sum('total');
        $saldoAntesDelPago = max(0, intval($total_presupuesto) - $abonadoAntesDelPago);
        if ($saldoAntesDelPago <= 0) {
            $presupuesto->pago_completado = true;
            $presupuesto->fecha_pago_completo = Carbon::now();
            $presupuesto->save();
            return ['estado' => 0, 'presupuesto_completado' => true, 'mensaje' => 'El presupuesto ya se encuentra completamente pagado.'];
        }
        if (intval($pago_actual) > $saldoAntesDelPago) {
            return ['estado' => 0, 'mensaje' => 'El abono supera el saldo pendiente de $'.number_format($saldoAntesDelPago, 0, ',', '.').'.'];
        }
        ...
        $nuevo_pago->id_prevision = $paciente->id_prevision;
        ...
        try {
            ...
            DB::rollBack();
            return ['estado' => 0, 'mensaje' => 'No fue posible registrar el pago: '.$e->getMessage()];
        }
        ...
        if($nuevo_pago->exists){
            ...
            $insumos_tratamientos = $fichaController->dame_insumos_tratamiento($req->id_paciente, $req->id_ficha_atencion)
                ->filter(function ($insumo) use ($req) {
                    return (int) $insumo->presupuesto === 1
                        && (int) ($insumo->urgencia ?? 0) === 0
                        && (!$req->id_presupuesto || (int) $insumo->id_presupuesto === (int) $req->id_presupuesto);
                })
                ->values();
            ...
            $total_general = intval($valores[0]) + intval($valores[1]) + intval($valores[2]) + intval($valores[3]);
            $total_con_descuento = max(0, $total_general - intval($descuentos));

            // La confirmacion final se hace con los abonos realmente persistidos,
            // no con los totales enviados por el navegador.
            $total_abonado = (int) PagosPresupuestoDental::where('id_presupuesto', $presupuesto->id)->sum('total');
            $presupuestoCompletado = $total_con_descuento > 0
            ...
            if($total_con_descuento <= $total_abonado){
                foreach($odontograma_paciente as $o){
                    if ($o->urgencia == 0 && $o->presupuesto == 1) {
                        $o->estado_pago = 'ok';
                        OdontogramaPaciente::whereKey($o->id)->update(['estado_pago' => 'ok']);
                        $o->clase = 'bg-success';
                    }
                }
                foreach ($insumos_tratamientos as $i) {
                    if ($i->urgencia == 0 && $i->presupuesto == 1) {
                        $i->estado_pago = 'ok';
                        $i->newQuery()->whereKey($i->id)->update(['estado_pago' => 'ok']);
                        $i->clase = 'bg-success';
                    }
                }
                foreach ($todos as $o) {
                    if ($o->presupuesto == 1 && (int) ($o->urgencia ?? 0) === 0) {
                        $o->estado_pago = 'ok';
                        ExamenesBocaGeneral::whereKey($o->id)->update(['estado_pago' => 'ok']);
                        $o->clase = 'bg-success';
                    }
                }
                // El pago se cierra, pero el presupuesto sigue clínicamente activo
                // hasta que todas sus prestaciones hayan sido atendidas.
                $presupuesto->pago_completado = true;
                $presupuesto->fecha_pago_completo = Carbon::now();
            ...
            return [
                'estado' => 1,
                'mensaje' => $presupuestoCompletado ? 'El presupuesto fue pagado completamente y se ha cerrado.' : 'Se ha registrado el pago correctamente',
                'presupuesto_completado' => $presupuestoCompletado,
    ...
    public function pago_urgencias(Request $req){
        ...
        try {
            ...
            $profesional = Profesional::where('id_usuario',Auth::user()->id)->first();
            if (!$profesional || !is_numeric($req->monto) || intval($req->monto) <= 0) {
                return ['estado' => 0, 'mensaje' => 'Profesional o monto de pago inválido.'];
            }
            ...
            $presupuesto = PresupuestosDental::where('id', $req->id_presupuesto)
                ...
                ->first();
            if (!$presupuesto) {
                return ['estado' => 0, 'mensaje' => 'No se encontró un presupuesto válido para este pago de urgencia.'];
            }
            // buscamos el presupuesto
            $total_presupuesto = $req->total_pago;
            ...
            if($nuevo_pago->save()){
                ...
                $respuesta = $this->actualizar_presupuesto_urg($req);
                $respuesta['mensaje'] = 'Se ha registrado el pago correctamente';
                $respuesta['pago_actual'] = $pago_actual;
                return $respuesta;
            }else{
                return ['estado' => 0, 'mensaje' => 'Ha ocurrido un problema'];
            }
    ...
    public function actualizar_presupuesto(Request $req){
        try {
            ...
            //code...
        // El convenio aplicado pertenece al presupuesto y es la fuente de verdad.
        // id_dcto queda sólo como fallback de compatibilidad para peticiones antiguas.
        $convenio = null;
        ...
        if(!$presupuesto){
            $presupuesto = PresupuestosDental::where('id_paciente', $req->id_paciente)
                ->where('id_ficha_atencion', $req->id_ficha_atencion)
                ->where('id_profesional', $profesional ? $profesional->id : null)
                ->orderByDesc('id')
                ->first();
        }

        if ($presupuesto && $profesional) {
            $idConvenioAplicado = (int) ($presupuesto->id_convenio_aplicado ?: $req->id_dcto);
            if ($idConvenioAplicado > 0) {
                $convenio = EmpresasConvenios::whereKey($idConvenioAplicado)
                    ->where('id_profesional', $profesional->id)
                    ->where('estado_convenio', 1)
                    ->first();
                $porcentaje_descuento = $convenio ? (int) $convenio->porcentaje : 0;
            }
        }

        if($presupuesto){
            // Contrato monetario único para todas las fichas odontológicas:
            // valor = bruto, valor_descuento = monto descontado, nuevo_valor = neto.
            $calcularMontos = function ($valorBruto) use ($porcentaje_descuento) {
                $bruto = max(0, (int) round($valorBruto));
                $descuento = $porcentaje_descuento > 0
                    ? (int) round($bruto * ($porcentaje_descuento / 100))
                    : 0;
                return [
                    'bruto' => $bruto,
                    'descuento' => min($bruto, max(0, $descuento)),
                    'neto' => max(0, $bruto - $descuento),
                ];
            };
            $id_ficha_atencion = $presupuesto->id_ficha_atencion;
            $descuentos = 0;
            // Reconciliar primero y usar esos estados persistidos como única fuente
            // para la tabla principal y el modal de reasignación.
            $this->reconciliarAsignacionPresupuestoDental($presupuesto, $profesional);
            $odontograma_paciente = $this->dame_odontograma_paciente(
                $req->id_paciente,
                $id_ficha_atencion,
            ...
            $valores = $this->dameValoresOdontograma($req->id_paciente, $id_ficha_atencion, $req->id_lugar_atencion, $profesional->id_tipo_especialidad, $presupuesto->id);
            ...
            $monto_piezas_ya_cubiertas = $tiene_reasignacion_persistida
                ? (int) $odontograma_paciente->where('estado_pago', 'ok')->sum(function ($pieza) use ($calcularMontos) {
                    $montos = $calcularMontos($pieza->valor);
                    return $montos['neto'];
                })
                : 0;
            $resto_insumos = max(0, $total_abonado - $monto_piezas_ya_cubiertas);
            ...
            foreach ($insumos_tratamientos as $i) {
                ...
                if ($tiene_reasignacion_persistida) {
                    // La conciliación ya determinó el estado usando valores netos
                    // con descuento; no volver a sobrescribirlo con montos brutos.
                    continue;
                }

                $valor_insumo_bruto = intval($i->valor) * max(1, intval($i->cantidad));
                $montosInsumo = $calcularMontos($valor_insumo_bruto);
                $valor_insumo = $montosInsumo['neto'];
            ...
            foreach($insumos_tratamientos as $i){
                if ((int) ($i->urgencia ?? 0) === 1 || (int) ($i->presupuesto ?? 0) !== 1) {
                    continue;
                }

                $subtotalInsumo = intval($i->valor) * max(1, intval($i->cantidad));
                $montosInsumo = $calcularMontos($subtotalInsumo);
                $i->valor_descuento = $montosInsumo['descuento'];
                $i->nuevo_valor = $montosInsumo['neto'];
                $descuentos += $montosInsumo['descuento'];
                $i->nombre_marca = $i->nombre_marca ? strtoupper($i->nombre_marca) : '';
            }


            // Sólo se descuenta del abono el monto que realmente pudo asignarse a
            // insumos, no el precio completo de un insumo nuevo aún impago.
            $monto_asignado_insumos = min($valor_insumos, max(0, $total_abonado - $monto_piezas_ya_cubiertas));
            $total_abonado_sin_insumos = $tiene_reasignacion_persistida
                ? $total_abonado
                : max(0, $total_abonado - $monto_asignado_insumos);
            ...
            foreach($odontograma_para_distribuir as $o){
                ...
                if($o->presupuesto == 1){
                    ...
                    $montosPiezaPago = $calcularMontos($o->valor);
                    $valorPiezaPago = $montosPiezaPago['neto'];
                    if($resto > 0 && $resto >= $valorPiezaPago){
                        $o->estado_pago = 'ok';
                        $resto -= $valorPiezaPago;

                    }else if($resto > 0 && $resto < $valorPiezaPago){
                        $o->estado_pago = 'incompleto';
                        $resto = 0;
                    }else if($resto <= 0){
                        $o->estado_pago = 'error';
            ...
            foreach($odontograma_paciente as $o){
                if ((int) ($o->urgencia ?? 0) === 1 || (int) ($o->presupuesto ?? 0) !== 1) {
                    continue;
                }
                $montosPieza = $calcularMontos($o->valor);
                $o->valor_descuento = $montosPieza['descuento'];
                $o->nuevo_valor = $montosPieza['neto'];
                $descuentos += $montosPieza['descuento'];
            }
            ...
            foreach($todos as $o){
                ...
                //     continue;
                // }
                if((int) ($o->presupuesto ?? 0) === 1){
                    $montosGrupo = $calcularMontos($o->valor);
                    $o->valor_descuento = $montosGrupo['descuento'];
                    $o->nuevo_valor = $montosGrupo['neto'];
                    $descuentos += $montosGrupo['descuento'];
                }

                if((int) ($o->presupuesto ?? 0) === 1 && !$tiene_reasignacion_persistida){
            ...
            $fc = new ficha_atencionController();
            $ordenes_trabajo_menor = $fc->dameOrdenesTrabajoMenor($paciente->id, $profesional->id, $presupuesto->id, $id_ficha_atencion);
            $ordenes_trabajo_mayor = $fc->dameOrdenesTrabajoMayor($paciente->id, $profesional->id, $presupuesto->id, $id_ficha_atencion);

            foreach($ordenes_trabajo_menor as $trabajo){
                if($trabajo->presupuesto == 1){
                    $total_lab += intval($trabajo->valor_prestacion);
                    $montosLab = $calcularMontos($trabajo->valor_prestacion);
                    $trabajo->valor_descuento = $montosLab['descuento'];
                    $trabajo->nuevo_valor = $montosLab['neto'];
                    $descuentos += $montosLab['descuento'];
                }
            }
            ...
            // $valores[3] ya contiene laboratorio. No volver a sumarlo al total.
            // Recalcular el total neto ahora que también conocemos el descuento de laboratorio.
            $total_general = intval($valores[0]) + intval($valores[1]) + intval($valores[2]) + intval($valores[3] ?? 0);
            $total_con_descuento = max(0, $total_general - intval(round($descuentos)));
            $suma_adeudado = $total_con_descuento;
            ...
            return [
                ...
                'presupuesto' => $presupuesto,
                'id_convenio_aplicado' => $presupuesto->id_convenio_aplicado,
                'porcentaje_descuento' => $porcentaje_descuento,
                'descuentos' => (int) round($descuentos),
            ];
            }else{
    ...
    public function actualizar_presupuesto_urg(Request $req){
        try {
            ...
            $profesional = Profesional::where('id_usuario',Auth::user()->id)->first();
            $paciente = Paciente::find($req->id_paciente);
            $presupuestoUrgencia = $profesional
                ? PresupuestosDental::whereKey($req->id_presupuesto)
                    ->where('id_paciente', $req->id_paciente)
                    ->where('id_profesional', $profesional->id)
                    ->first()
                : null;
            // El convenio de urgencia se persiste por separado. Si todavía no se
            // ha definido, se asigna automáticamente desde la previsión vigente
            // del paciente, sin copiar el descuento del presupuesto clínico.
            $nombrePrevision = trim((string) optional(optional($paciente)->prevision)->nombre);
            $consultaConvenioUrgencia = function () use ($profesional, $nombrePrevision) {
                if (!$profesional || $nombrePrevision === '') {
                    return EmpresasConvenios::query()->whereRaw('1 = 0');
                }

                return EmpresasConvenios::query()
                    ->where('id_profesional', $profesional->id)
                    ->where('estado_convenio', 1)
                    ->whereRaw('LOWER(TRIM(nombre_convenio)) = ?', [mb_strtolower($nombrePrevision)]);
            };

            $convenioUrgencia = null;
            if ($presupuestoUrgencia && $presupuestoUrgencia->id_convenio_urgencia_aplicado) {
                $convenioUrgencia = $consultaConvenioUrgencia()
                    ->whereKey($presupuestoUrgencia->id_convenio_urgencia_aplicado)
                    ->first();

                if (!$convenioUrgencia) {
                    $presupuestoUrgencia->id_convenio_urgencia_aplicado = null;
                    $presupuestoUrgencia->save();
                }
            }

            if ($presupuestoUrgencia && !$convenioUrgencia) {
                $convenioUrgencia = $consultaConvenioUrgencia()->orderByDesc('id')->first();
                if ($convenioUrgencia) {
                    $presupuestoUrgencia->id_convenio_urgencia_aplicado = $convenioUrgencia->id;
                    $presupuestoUrgencia->save();
                }
            }
            $porcentajeDescuentoUrgencia = $convenioUrgencia ? (float) $convenioUrgencia->porcentaje : 0;
            $calcularNetoUrgencia = function ($valor) use ($porcentajeDescuentoUrgencia) {
                return max(0, (int) round((float) $valor * (1 - $porcentajeDescuentoUrgencia / 100)));
            };
            $fichaController = new ficha_atencionController;
            $insumos_tratamientos = $fichaController->dame_insumos_tratamiento($req->id_paciente, $req->id_ficha_atencion);
            $insumos_por_pagar = 0;
            ...
            $pagos_urgencia = PagosUrgenciaDental::select('pagos_urgencia_dental.*', 'profesionales.nombre as nombre_profesional','profesionales.apellido_uno','profesionales.apellido_dos')
                ...
                ->where('id_profesional', $profesional->id)
                ->when($req->id_presupuesto, function ($query, $idPresupuesto) {
                    $query->where('pagos_urgencia_dental.id_presupuesto', $idPresupuesto);
                })
            ...
            foreach ($insumos_tratamientos as $i) {
                ...
                $valor_insumo_bruto = intval($i->valor) * intval($i->cantidad);
                $valor_insumo = $calcularNetoUrgencia($valor_insumo_bruto);
                $i->valor_descuento = $valor_insumo_bruto - $valor_insumo;
                $i->nuevo_valor = $valor_insumo;
            ...
            foreach($insumos_tratamientos as $i){
                if($i->urgencia == 1 && $i->presupuesto == 1){
                    $a_pagar = $calcularNetoUrgencia($i->cantidad * $i->valor);
            ...
            foreach($odontograma_paciente as $o){
                ...
                $valorPiezaUrgencia = $calcularNetoUrgencia($o->valor);
                $o->valor_descuento = intval($o->valor) - $valorPiezaUrgencia;
                $o->nuevo_valor = $valorPiezaUrgencia;
                if ($pagos >= $valorPiezaUrgencia) {
                    $o->estado_pago = 'ok';
                    $pagos -= $valorPiezaUrgencia;
                } elseif ($pagos > 0 && $pagos < $valorPiezaUrgencia) {
                    $o->estado_pago = 'incompleto';
            ...
            foreach($odontograma_paciente as $o){
                if($o->urgencia == 1 && $o->presupuesto == 1){
                    $piezas_por_pagar += $calcularNetoUrgencia($o->valor);
                }
            }
            ...
            return [
                ...
                'porcentaje_descuento' => $porcentajeDescuentoUrgencia,
                'id_convenio_urgencia' => optional($convenioUrgencia)->id,
                'convenio_urgencia' => optional($convenioUrgencia)->nombre_convenio,
                'descuentos' => ($insumos_tratamientos->sum(fn ($i) => (int) ($i->valor_descuento ?? 0)))
                    + ($odontograma_paciente->sum(fn ($o) => (int) ($o->valor_descuento ?? 0))),
    ...
    public function contexto_agendamiento(Request $request)
    {
        $request->validate([
            'id_paciente' => 'required|integer|exists:pacientes,id',
        ...
        $profesional = Profesional::where('id_usuario', Auth::id())->first();
        if (!$profesional || (int) $profesional->id !== (int) $request->id_profesional) {
            return response()->json(['mensaje' => 'Profesional no autorizado.'], 403);
        }

        $presupuestos = PresupuestosDental::where('id_paciente', $request->id_paciente)
            ->where('id_profesional', $profesional->id)
            ->where('estado', 1)
            ->orderByDesc('id')
            ->get()
            ->map(function ($presupuesto) use ($profesional) {
                $pendientes = OdontogramaPaciente::where('id_presupuesto', $presupuesto->id)
                    ->where('presupuesto', 1)
                    ->where('urgencia', 0)
                    ...
                    ->get()
                    ->map(function ($tratamiento) use ($profesional) {
                        $catalogo = (int) $profesional->id_tipo_especialidad === 16
                            ? \App\Models\TratamientosImplantologia::where('descripcion', $tratamiento->tratamiento)->first()
                            : DiagnosticosDental::where('descripcion', $tratamiento->tratamiento)->first();

                        return [
                            'id' => $tratamiento->id,
                            'tipo' => 'pieza',
                            'pieza' => $tratamiento->pieza,
                            'tratamiento' => $tratamiento->tratamiento ?: $tratamiento->descripcion,
                            'progreso' => (int) ($tratamiento->progreso ?? ((int) $tratamiento->estado === 1 ? 100 : 0)),
                            'bloques' => max(1, (int) ($catalogo->cantidad_bloques ?? 1)),
                        ];
                    })->values();

                $gruposPendientes = ExamenesBocaGeneral::where('id_presupuesto', $presupuesto->id)
                    ->where('id_paciente', $presupuesto->id_paciente)
                    ...
                    ->get()
                    ->map(function ($grupo) use ($profesional) {
                        $catalogo = (int) $profesional->id_tipo_especialidad === 16
                            ? DiagnosticosDental::where('descripcion', $grupo->diagnostico_tratamiento)->first()
                            : \App\Models\TratamientosImplantologia::where('descripcion', $grupo->diagnostico_tratamiento)->first();

                        return [
                            'id' => $grupo->id,
                            'tipo' => 'grupo',
                            'nombre' => $grupo->grupo ?: ($grupo->pieza ?: 'Grupo de piezas'),
                            'tratamiento' => $grupo->diagnostico_tratamiento ?: 'Tratamiento dental',
                            'progreso' => (int) ($grupo->progreso ?? 0),
                            'bloques' => max(1, (int) ($catalogo->cantidad_bloques ?? 1)),
                        ];
                    });

                $pendientes = $pendientes->concat($gruposPendientes)->values();

                return ['id' => $presupuesto->id, 'pendientes' => $pendientes];
            })
            ->filter(fn ($presupuesto) => $presupuesto['pendientes']->isNotEmpty())
            ->values();
    ...
    private function recalcularPagosYDescuentos($id_paciente, $id_ficha, $id_lugar, $id_especialidad, $monto_abonado, $id_convenio = 0)
    {
        ...
        // Pagos y estados pertenecen al presupuesto clínico. Los registros del
        // presupuesto express de urgencia se calculan y pagan en su flujo propio.
        $odontograma = collect($odontograma)->filter(function ($item) {
            return (int) data_get($item, 'presupuesto', 0) === 1
                && (int) data_get($item, 'urgencia', 0) === 0;
        })->values();
        $insumos = collect($insumos)->filter(function ($item) {
            return (int) data_get($item, 'presupuesto', 0) === 1
                && (int) data_get($item, 'urgencia', 0) === 0;
        })->values();
    ...
    public function dame_pagos_presupuesto($id_presupuesto){
        $pagos = PagosPresupuestoDental::with('prevision')
            ->where('id_presupuesto', $id_presupuesto)
            ->orderBy('fecha_pago')
            ->orderBy('id')
            ->get()
            ->each(function ($pago) {
                $pago->convenio_nombre = optional($pago->prevision)->nombre ?: 'Sin convenio';
            });
        return $pagos;
    }
    ...
    public function dame_prestaciones_presupuesto(Request $req)
    {
        ...
        if (!$presupuesto || !$profesional) {
            return response()->json(['mensaje' => 'No se encontró el presupuesto clínico.'], 404);
        }

        // Al retirar el descuento se reconstruye el presupuesto desde una única
        // fuente de verdad: prestaciones clínicas del presupuesto actual y
        // abonos realmente persistidos. Urgencias nunca participa en este cálculo.
        $total_abonado = (int) PagosPresupuestoDental::where('id_presupuesto', $presupuesto->id)->sum('total');
        $fcRecalculo = new ficha_atencionController;
        $odontogramaRecalculado = collect($this->dame_odontograma_paciente(
            $req->id_paciente,
            $presupuesto->id_ficha_atencion,
            $req->id_lugar_atencion,
            $profesional->id_tipo_especialidad,
            $presupuesto->id
        ))->filter(fn ($item) => (int) data_get($item, 'presupuesto', 0) === 1
            && (int) data_get($item, 'urgencia', 0) === 0
            && (int) data_get($item, 'id_presupuesto', 0) === (int) $presupuesto->id)->values();
        $insumosRecalculados = collect($fcRecalculo->dame_insumos_tratamiento(
            $req->id_paciente,
            $presupuesto->id_ficha_atencion
        ))->filter(fn ($item) => (int) data_get($item, 'presupuesto', 0) === 1
            && (int) data_get($item, 'urgencia', 0) === 0
            && (int) data_get($item, 'id_presupuesto', 0) === (int) $presupuesto->id)->values();
        $gruposRecalculados = collect($this->dameTratamientosBocaGeneral($presupuesto->id_ficha_atencion))
            ->filter(fn ($item) => (int) data_get($item, 'presupuesto', 0) === 1
                && (int) data_get($item, 'urgencia', 0) === 0
                && (int) data_get($item, 'id_presupuesto', 0) === (int) $presupuesto->id)->values();

        $saldoDistribuir = $total_abonado;
        $asignarEstado = function ($item, int $valor, string $modelo) use (&$saldoDistribuir) {
            if ($saldoDistribuir >= $valor) {
                $estado = 'ok';
                $saldoDistribuir -= $valor;
            } elseif ($saldoDistribuir > 0) {
                $estado = 'incompleto';
                $saldoDistribuir = 0;
            } else {
                $estado = 'error';
            ...
            $item->nuevo_valor = $valor;
            $modelo::whereKey($item->id)->update(['estado_pago' => $estado]);
        };

        foreach ($insumosRecalculados->sortBy(fn ($item) => (int) $item->valor * max(1, (int) $item->cantidad)) as $item) {
            $asignarEstado($item, (int) $item->valor * max(1, (int) $item->cantidad), InsumosTratamientosDental::class);
        }
        foreach ($odontogramaRecalculado as $item) {
            $asignarEstado($item, (int) $item->valor, OdontogramaPaciente::class);
        }
        foreach ($gruposRecalculados as $item) {
            $asignarEstado($item, (int) $item->valor, ExamenesBocaGeneral::class);
        }

        $totalGrupos = (int) $gruposRecalculados->sum('valor');
        $totalPiezas = (int) $odontogramaRecalculado->sum('valor');
        $totalInsumos = (int) $insumosRecalculados->sum(fn ($item) => (int) $item->valor * max(1, (int) $item->cantidad));
        $valoresRecalculados = [$totalGrupos, $totalPiezas, $totalInsumos, 0];
        $totalRecalculado = array_sum($valoresRecalculados);

        return [
            'odontograma' => $odontogramaRecalculado,
            'insumos' => $insumosRecalculados,
            'todos' => $gruposRecalculados,
            'valores' => $valoresRecalculados,
            'descuentos' => 0,
            'total_general' => $totalRecalculado,
            'total_abonado' => $total_abonado,
            'saldo_a_favor' => max(0, $total_abonado - $totalRecalculado),
            'saldo_pendiente' => max(0, $totalRecalculado - $total_abonado),
            'convenio_aplicado' => false,
            'total_lab' => 0,
        ];
    ...
    public function generar_pdf_prot_period(Request $req){
        try {
            $id_ficha_atencion = $req->id_ficha_atencion;
            $ficha_atencion = FichaAtencion::find($id_ficha_atencion);
            $lugar_atencion = LugarAtencion::find($ficha_atencion->id_lugar_atencion);
            ...
            $det_cir = $req->det_cir;
            $nombre_tons = $req->nombre_tons;
            $prot_pieza_imp = $req->prot_pieza_period;
            //code...
            // Renderizar la vista del presupuesto dental
            $pdf = Pdf::loadView('atencion_odontologica.PDF.protocolo_periodonto_dental',compact(
                'id_paciente',
                'nombre_cir',
                'nombre_cir_nuevo',
                'nombre_anest',
            ...
            // Guardar el PDF en la carpeta public
            $fileName = 'protocolo_dental_' . $req->id_paciente . '.pdf';
            $filePath = public_path('reportes/' . $fileName);
            file_put_contents($filePath, $pdf->output());
    ...
    public function dame_tratamientos_pieza_impl(Request $req){
        $profesional = Profesional::where('id_usuario', Auth::id())->first();
        if (!$profesional) {
            return response()->json(['tratamientos' => [], 'error' => 'Profesional no encontrado.'], 403);
        }
        ...
        $tratamientos = OdontogramaPaciente::where('id_paciente', $req->id_paciente)
            ...
            ->where('tratamiento', '<>', '')
            // Una prestación pendiente puede no tener estado todavía y una
            // prestación ya iniciada usa estado 2. Ambas deben poder
            // seleccionarse para registrar el procedimiento implantológico.
            ->where(function ($query) {
...

    {
        ...
        $relacionTons = ProfesionalTons::where('id', $request->id_profesional_tons)
            ->where('id_profesional', $profesional->id)
            ->where('estado', 2)
            ->first();

        if (!$relacionTons) {
            return redirect()->back()->with('error', 'Debe seleccionar una TONS activa asociada a su perfil.');
        }
        ...
        $control_laboratorio->id_profesional_tons = $relacionTons->id;
    ...
    public function getArticuloHomeopatia(Request $request){
        $search = $request->search;

        // Base query para homeopatía
        $query = ArticuloHomeopatia::where('grupo', 'HOMEOPATIA')
                                  ->orderby('nombre', 'asc')
                                  ->select('id', 'nombre', 'droga', 'cant_comp', 'dosis', 'cont_rec');

        if ($search == '') {
            $employees = $query->limit(15)->get();
        } else {
            $employees = $query->where('nombre', 'like', $search . '%')->limit(15)->get();
        }
        ...
        foreach ($employees as $employee) {
            $response[] = array(
                ...
                "droga" => $employee->droga ?? $employee->cont_rec, // usar cont_rec como backup si droga es null
                "control" => $employee->dosis ?? '1', // usar dosis como control o valor por defecto
                "cant_comp" => $employee->cant_comp
            );
        }
    ...
    public function getDosisHomeopatia(Request $request){
        $articulo = ArticuloHomeopatia::where('id', $request->id_medicamento)->first();
        $dosis = array();
        if($articulo){
            $dosis[] = array(
                "dosis" => $articulo->cont_rec,
                "control" => $articulo->dosis, // usar dosis como control o valor por defecto
            );
        }else{
            $dosis[] = array(
                "dosis" => '1',
                "control" => '1', // usar dosis como control o valor por defecto
            );
        }
        ...
        if($pieza->save()){
            if ($pieza->id_presupuesto) {
                ...
                if ($presupuesto) {
                    ...
                    /*
                     * Un progreso clínico de 100% NO debe cerrar, desvincular ni
                     * ocultar el presupuesto dental.
                     *
                     * "estado" del presupuesto se mantiene activo para conservarlo
                     * visible en:
                     * - Tratamiento (el registro terminado queda en solo lectura)
                     * - Presupuesto clínico
                     * - Pagos y estados
                     * - Historial clínico
                     *
                     * El cierre clínico se informa por separado mediante
                     * $presupuestoClinicamenteFinalizado. Los selectores que deban
                     * mostrar sólo pendientes deben filtrar por progreso/estado del
                     * tratamiento, no desactivar el presupuesto completo.
                     */
                    $presupuesto->estado = 1;
                    $presupuesto->save();
                }
            ...
            /*
             * Si el tratamiento pertenece a un presupuesto, refrescar usando la
             * ficha/lugar históricos de ese presupuesto. Esto evita que una pieza
             * desaparezca al llegar a 100% o al continuar el tratamiento en una
             * atención posterior.
             */
    ...
    public function guardarProgresoGrupoDental(Request $request)
    {
        $request->validate([
            'id_grupo' => 'required|integer|exists:examenes_boca_general,id',
            'id_paciente' => 'required|integer',
            'id_presupuesto' => 'required|integer|exists:presupuestos_dental,id',
            'progreso' => 'required|integer|in:0,25,50,75,100',
        ]);

        $profesional = Profesional::where('id_usuario', Auth::id())->firstOrFail();
        $presupuesto = PresupuestosDental::whereKey($request->id_presupuesto)
            ->where('id_paciente', $request->id_paciente)
            ->where('id_profesional', $profesional->id)
            ->firstOrFail();

        $grupo = ExamenesBocaGeneral::whereKey($request->id_grupo)
            ->where('id_paciente', $request->id_paciente)
            ->where('id_profesional', $profesional->id)
            ->where('id_presupuesto', $presupuesto->id)
            ->where('presupuesto', 1)
        ...
        $progreso = (int) $request->progreso;

        $grupo->progreso = $progreso;
        $grupo->terminado = $progreso >= 100 ? 1 : 0;

        // La agenda dental usa el campo `atendido` como fuente de verdad
        // para determinar si un tratamiento por grupo/maxilar está finalizado.
        // Mantenerlo sincronizado con el progreso evita que un grupo al 100%
        // siga apareciendo como "PENDIENTE DE ATENCIÓN".
        $grupo->atendido = $progreso >= 100 ? 1 : 0;

        $grupo->save();

        $grupos = $this->dameTratamientosBocaGeneral(
            $presupuesto->id_ficha_atencion,
            null,
            $profesional->id
        )->where('id_presupuesto', $presupuesto->id)->values();
    ...
    public function guardarCarasTratamientoUrgencia(Request $request)
    {
        $request->validate([
            'id_tratamiento' => 'required|integer|exists:odontogramas_pacientes,id',
            'id_paciente' => 'required|integer',
    ...
    public function getTratamientoImplantologia(Request $request){
        $search = $request->search;
        if ($search == '') {
            $query = TratamientosImplantologia::orderby('descripcion', 'asc')->select('id', 'descripcion', 'valor', 'uco', 'tipo_examen');
        } else {
            $query = TratamientosImplantologia::orderby('descripcion', 'asc')->select('id', 'descripcion', 'valor', 'uco', 'tipo_examen')->where('descripcion', 'like', $search . '%');
        }
        if (!$request->boolean('catalogo')) {
            $query->limit(15);
        }
        ...
        foreach ($employees as $employee) {
            ...
            $employee->descripcion = mb_strtoupper($employee->descripcion, 'UTF-8');
            $response[] = array("value" => $employee->id, "label" => $employee->descripcion,"descripcion" => $employee->descripcion, "control" => $employee->tipo_examen,'valor' => $employee->valor, 'uco' => $employee->uco);
    ...
    public function horasDisponiblesProfesionalLugarAtencionBuscador(Request $request){
        ...
        // Buscar solo agendas normales (excluyendo exámenes: tipo_agenda != 4)
        // Tipos de agenda: 1=Atención General, 2=Atención Dental, 3=Telemedicina, 4=Exámenes
        $profesional_horarios = ProfesionalHorario::where('id_profesional',$request->id_profesional)
                                                ->where('id_lugar_atencion',$request->id_lugar_atencion)
                                                ->where('dia','like','%'.$dia_semana.'%')
                                                ->whereIn('tipo_agenda', [1, 2, 3])  // Solo agendas normales, excluyendo exámenes
                                                ->orderBy('dia', 'ASC')
                                                ->first();
    ...
    public function insertPaciente(Request $request){
        try {
            ...
            // Validar si existe el paciente por el RUT
            $validacion_rut = Paciente::where('rut', $request->rut)->first();

            if(!$validacion_rut)
            {
                // validamos si la direccion y el numero ya existe
                $validacion_direccion = Direccion::where('direccion', $request->direccion)
                                                ->where('numero_dir', $request->numero ?? '')
                                                ->where('id_ciudad', $request->comuna ?? null)
                                                ->first();
                if($validacion_direccion){
                    $direccion = $validacion_direccion;
                }else{
                    // Crear dirección primero
                    $direccion = new Direccion();
                    $direccion->direccion = $request->direccion ?? '';
                    $direccion->numero_dir = $request->numero ?? '';
                    $direccion->id_ciudad = $request->comuna ?? null;
                    $direccion->save();
                }
                ...
                $paciente->sexo = $request->sexo ?? 'M';

                // Convertir fecha de nacimiento según el formato recibido
                if (!empty($request->fecha_nacimiento)) {
                    if (strpos($request->fecha_nacimiento, '/') !== false) {
                    ...
                    } elseif (strpos($request->fecha_nacimiento, '-') !== false) {
                        // Si tiene formato con guiones, validar si es d-m-Y o Y-m-d
                        $partes = explode('-', $request->fecha_nacimiento);
                        ...
                        $fechaConvertida = $request->fecha_nacimiento;
                    }
                    $paciente->fecha_nac = $fechaConvertida;
                }

                $paciente->id_prevision = $request->id_prevision ?? 1;
                ...
                if($paciente->save()){
                    return [
                        'estado' => 1,
                        'mensaje' => 'Paciente guardado de forma correcta',
                        'msj' => 'Paciente guardado de forma correcta',
                    ...
                    return [
                        ...
                        'msj' => 'Error al guardar el paciente'
                    ];
                }
            } else {
                ...
                return [
                    ...
                    'msj' => 'El paciente ya existe en el sistema',
                    'id_paciente' => $validacion_rut->id
                ];
            }
        } catch (\Exception $e) {
            return [
    ...
    public function getFrecuenciaHomeopatia(Request $request)
    {

        $articulo = ArticuloHomeopatia::where('id', $request->id_dosis)->first();

        //$dosis = RecetaDosis::where('id', $request->id_dosis)->get();
        $frecuencias = RecetaDosisHomeopatia::where('cod_parent', $articulo->dosis)->get();
        //dd($frecuencias);
        return json_encode($frecuencias);
    }

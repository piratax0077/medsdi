...
class MobileTwoFactorController extends Controller
{
    ...
    public function pending(Request $request)
    {
        ...
        // Expira pendientes antiguos en una sola operación indexable.
        LoginApprovalChallenge::where('user_id', $userId)
            ->where('status', 'pending')
            ->where('expires_at', '<=', $now)
            ->update(['status' => 'expired']);

        $challenges = LoginApprovalChallenge::where('user_id', $userId)
            ->where('status', 'pending')
            ->where('expires_at', '>', $now)
            ->orderByDesc('id')
            ->limit(20)
